...
anything useful. For example, NailGun needs to know the HOST of the remote
...
class BaseServerConfig(object):
    ...
    :param host: A string. The HOST of a server. For example:
    ...
    def __init__(self, host, auth=None, key=None, version=None):
        self.host = host
        if auth is not None:
            self.auth = auth
        if key is not None:
...
class ServerConfig(BaseServerConfig):
    ...
    _xdg_config_dir = 'railgun'
    _xdg_config_file = 'server_configs.json'

    def __init__(self, host, auth=None, key=None, version=None):
        super(ServerConfig, self).__init__(host, auth, key, version)
    ...
    def get_client_kwargs(self):
        ...
        as kwargs via the ``**`` operator. For example::
            ...
            client.get(cfg.host + '/api/v2', **cfg.get_client_kwargs())
        ...
        also be written like this::
            ...
            cfg = ServerConfig.get()
            client.get(cfg.host + '/api/v2', auth=cfg.auth, verify=cfg.verify)
